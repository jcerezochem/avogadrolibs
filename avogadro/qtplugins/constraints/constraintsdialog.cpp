    ...
    m_model(new ConstraintsModel)
{
  ...
  connect(ui->comboType, SIGNAL(currentIndexChanged(int)), this,
          SLOT(changeType(int)));
  connect(ui->comboMode, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setScanMode(int)));
...
bool ConstraintsDialog::isScanMode() const
{
  return ui->comboMode->currentIndex() == 1;
}
...
void ConstraintsDialog::setScanMode(int mode)
{
  const bool enabled = (mode == 1);
  ui->editScanInitial->setEnabled(enabled);
  ui->editScanEnd->setEnabled(enabled);
  ui->editScanSteps->setEnabled(enabled);

  if (!enabled) {
    syncScanEditorValue(ui->editValue->value());
    ui->editScanSteps->setValue(2);
  }
}
